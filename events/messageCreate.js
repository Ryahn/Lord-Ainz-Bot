const config = require('../config/config.json'),
    ...
    Log = require('../structures/models/log'),
    Member = require('../structures/models/member'),
    ...
    async run(message, client) {
        try {
            ...
            if (
                !message.guild ||
                message.author.bot ||
                !message.content.startsWith(this.bot.prefix)
            )
                return;
            ...
            //     guildData = await Guild.findOne({ id: message.guild.id });
            ...
            // ) {
            //     client.logger.info(
            //         `${chalk.cyanBright("[COMMAND]")} ${message.author.tag} used ${cmd.name
            //         } - Missing permissions`
            //     );
